...
function collectEnvSets(metas: PackageMeta[]): Array<[string, string]> {
...
function buildEnvSetCache(metas: PackageMeta[]): string {
  const lines = collectEnvSets(metas).map(([key, value]) => `${key}=${value}`);
  return `${lines.join("\n")}${lines.length ? "\n" : ""}`;
}
...
export async function regenerateEnvScripts(root: string): Promise<void> {
  ...
  try {
    ...
    await writeText(join(dirs.temp, "cache-env-sets.txt"), buildEnvSetCache(metas));
  } catch {
