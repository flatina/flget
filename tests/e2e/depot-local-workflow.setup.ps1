...
# Create a second install root (depot source) with a pre-installed scoop package
$depotRoot = Join-Path (Split-Path -Parent $InstallRoot) "depot-source"
Ensure-Directory $depotRoot

# Copy runtime files to depot root
...
# Pre-install a fake scoop package into depot root
$pkgDir = Join-Path $depotRoot "scoop\fldemo\current"
Ensure-Directory $pkgDir
Set-Content -LiteralPath (Join-Path $pkgDir "fldemo.cmd") -Encoding ASCII -Value "@echo depot-fldemo-1.0.0"

$metaDir = Join-Path $depotRoot "scoop\fldemo"
Set-Content -LiteralPath (Join-Path $metaDir "flget.meta.json") -Encoding UTF8 -Value (@{
  displayName = "fldemo"
